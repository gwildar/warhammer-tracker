...
for (const rule of SPECIAL_RULES) {
  if (rule.chargeMod) {
    ...
    if (rule.aliases) {
      for (const a of rule.aliases) CHARGE_MOD_RULES.set(a.toLowerCase(), rule.chargeMod)
    }
  }
}
...
export function renderChargeContext(army) {
  ...
  return `
    <div class="bg-wh-surface rounded-lg border border-wh-phase-combat/30 p-4 mb-4">
      ...
      <div class="space-y-1">
        ${rows.map(({ u, groundCharge, flyCharge, hasFly, chargeMods }) => `
            <div class="text-sm py-1 px-2 rounded bg-wh-card">
              <div class="flex justify-between items-center">
                <div class="flex flex-wrap items-center gap-1">
                  ...
                  ${chargeMods.map(m => {
                    const colors = { green: 'text-wh-phase-movement bg-wh-phase-movement/10', orange: 'text-orange-400 bg-orange-400/10' }
