...
function parseUnit(raw, category) {
  ...
  if (Array.isArray(raw.armor)) {
    unit.armour = raw.armor.filter((a) => a.active).map((a) => a.name_en);
  }
  ...
  const gearParts = [...unit.equipment, ...unit.armour].flatMap((e) =>
    e.split(",").map((s) => s.trim()),
  );
  for (const part of gearParts) {
    if (
      MAGIC_ITEM_NAMES.has(part.toLowerCase()) &&
      !unit.magicItems.includes(part)
    ) {
  ...
  unit.hasLoreFamiliar = [...unit.magicItems, ...unit.equipment].some((item) =>
    item.toLowerCase().includes("lore familiar"),
  );
  ...
  if (Array.isArray(raw.mounts)) {
    ...
    if (activeMount) {
      ...
      if (Array.isArray(activeMount.options)) {
        if (
          activeMount.options.some(
            (o) => o.active && o.name_en?.toLowerCase() === "barding",
          )
        ) {
          unit.hasBarding = true;
        }
  ...
  if (unit.specialRules) {
    ...
    for (const rule of rules) {
      ...
      if (
        ...
        !unit.factionLores.includes(LORE_NAME_TO_KEY[cleaned])
      ) {
...
export function getShootingUnits(army) {
  ...
  return army.units.filter((u) => {
    const allText = [...u.equipment, ...u.armour, u.specialRules || ""].map(
      (e) => e.toLowerCase(),
    );
    const hasRangedGear = allText.some((g) =>
      missileKeywords.some((k) => g.includes(k)),
    );
